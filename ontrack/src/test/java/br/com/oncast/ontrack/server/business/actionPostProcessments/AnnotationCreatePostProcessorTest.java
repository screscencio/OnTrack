...
public class AnnotationCreatePostProcessorTest {
	...
	public void setup() throws Exception {
		...
		when(action.getAnnotation(projectContext, actionContext)).thenReturn(annotation);
		...
		when(projectContext.getProjectRepresentation()).thenReturn(projectRepresentationMock);
	}
	...
	@Test(expected = UnableToPostProcessActionException.class)
	public void shouldThrowUnableToCompleteActionExceptionWhenAnnotationCreationFails() throws Exception {
		when(action.getAnnotation(projectContext, actionContext)).thenThrow(new UnableToCompleteActionException(""));
		postProcess();
	}
	...
	private void postProcess() throws UnableToPostProcessActionException {
		new AnnotationCreatePostProcessor(persistenceService).process(action, actionContext, projectContext);
	}
