...
public class ScopeInsertParentAction_ReleaseCreationTest {
	...
	public void shouldBindScopeToRelease() throws UnableToCompleteActionException {
		...
		new ScopeInsertParentAction(scope.getId(), SCOPE_DESCRIPTION + "R1").execute(context, actionContext);
	...
	public void shouldCreateNewReleaseIfItDoesNotExist() throws UnableToCompleteActionException, ReleaseNotFoundException {
		...
		final Scope scope = rootScope.getChild(2);
		new ScopeInsertParentAction(scope.getId(), SCOPE_DESCRIPTION + releaseDescription).execute(context, actionContext);
	...
	public void shouldNotCreateNewReleaseIfItAlreadyExist() throws UnableToCompleteActionException, ReleaseNotFoundException {
		...
		final Scope scope = rootScope.getChild(2);
		new ScopeInsertParentAction(scope.getId(), SCOPE_DESCRIPTION + release.getDescription()).execute(context, actionContext);
	...
	public void rollbackShouldDeletePreviouslyCreatedRelease() throws UnableToCompleteActionException, ReleaseNotFoundException {
		...
		final Scope scope = rootScope.getChild(2);
		final ModelAction rollbackAction = new ScopeInsertParentAction(scope.getId(), SCOPE_DESCRIPTION + releaseDescription).execute(context, actionContext);
	...
	public void shouldDeletePreviouslyCreatedReleaseAndRecreateItAfterMultiplesUndoAndRedo() throws UnableToCompleteActionException, ReleaseNotFoundException {
		...
		doUserAction(new ScopeInsertParentAction(scope.getId(), SCOPE_DESCRIPTION + releaseDescription));
	...
	private void doUserAction(final ModelAction action) throws UnableToCompleteActionException {
		actionExecutionManager.doUserAction(UserActionTestUtils.create(action, actionContext));
	}
