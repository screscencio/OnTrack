...
public class Tree extends Widget implements HasTreeItems, HasWidgets, HasAnimation, HasAllKeyHandlers, HasAllFocusHandlers, HasSelectionHandlers<TreeItem>,
	...
	static class ImageAdapter {
		...
		public ImageAdapter(final Resources resources) {
	...
	protected TreeItem curSelection;
	 ...
	 */
	@Override
	public TreeItem addItem(final SafeHtml itemHtml) {
		return root.addItem(itemHtml);
	}
	 ...
	 */
	@Override
	public TreeItem addItem(final Widget widget) {
	...
	@Override
	public final HandlerRegistration addOpenHandler(final OpenHandler<TreeItem> handler) {
	 ...
	 */
	@Override
	public TreeItem addTextItem(final String itemText) {
	...
	public boolean remove(final Widget w) {
		// Validate.
		final TreeItem item = childWidgets.get(w);
	...
	public void removeItem(final IsTreeItem isItem) {
		if (isItem != null) {
			final TreeItem item = isItem.asTreeItem();
			removeItem(item);
		}
	}
	...
	public Iterator<TreeItem> treeItemIterator() {
		final List<TreeItem> accum = new ArrayList<TreeItem>();
	...
	void adopt(final Widget widget, final TreeItem treeItem) {
	...
	void orphan(final Widget widget) {
	 ...
	 */
	protected void collectElementChain(final ArrayList<Element> chain, final Element hRoot, final Element hElem) {
		if ((hElem == null) || (hElem == hRoot)) { return; }

		collectElementChain(chain, hRoot, DOM.getParent(hElem));
		chain.add(hElem);
	}

	protected boolean elementClicked(final Element hElem) {
		final ArrayList<Element> chain = new ArrayList<Element>();
	...
	protected TreeItem findItemByChain(final ArrayList<Element> chain, final int idx, final TreeItem root) {
		if (idx == chain.size()) { return root; }

		final Element hCurElem = chain.get(idx);
		for (int i = 0, n = root.getChildCount(); i < n; ++i) {
			final TreeItem child = root.getChild(i);
			if (child.getElement() == hCurElem) {
				final TreeItem retItem = findItemByChain(chain, idx + 1, root.getChild(i));
	...
	/**
	 * Get the top parent above this {@link TreeItem} that is in closed state. In other words, get the parent that is guaranteed to be visible.
	 * 
	 * @param item
	 * @return the closed parent, or null if all parents are opened
	 */
	private TreeItem getTopClosedParent(final TreeItem item) {
	...
	private void init(final ImageAdapter images, final boolean useLeafImages) {
	...
	protected void maybeCollapseTreeItem() {

		final TreeItem topClosedParent = getTopClosedParent(curSelection);
	...
	protected void maybeExpandTreeItem() {
	...
	private void moveFocus() {
		final Focusable focusableWidget = curSelection.getFocusable();
		if (focusableWidget != null) {
			...
			final int containerLeft = getAbsoluteLeft();
			final int containerTop = getAbsoluteTop();

			final int left = DOM.getAbsoluteLeft(selectedElem) - containerLeft;
			final int top = DOM.getAbsoluteTop(selectedElem) - containerTop;
			final int width = DOM.getElementPropertyInt(selectedElem, "offsetWidth");
			final int height = DOM.getElementPropertyInt(selectedElem, "offsetHeight");
	 ...
	 */
	protected void moveSelectionDown(final TreeItem sel, final boolean dig) {
		if (sel == root) { return; }

		// Find a parent that is visible
		final TreeItem topClosedParent = getTopClosedParent(sel);
		if (topClosedParent != null) {
	 ...
	 */
	protected void moveSelectionUp(final TreeItem sel) {
		// Find a parent that is visible
		final TreeItem topClosedParent = getTopClosedParent(sel);
		if (topClosedParent != null) {
	...
	private void setImages(final ImageAdapter images, final boolean useLeafImages) {
		this.images = images;
		this.useLeafImages = useLeafImages;

		if (!useLeafImages) {
			final Image image = images.treeLeaf().createImage();
			DOM.setStyleAttribute(image.getElement(), "visibility", "hidden");
			RootPanel.get().add(image);
			final int size = image.getWidth() + TreeItem.IMAGE_PAD;
			image.removeFromParent();
			indentValue = (size) + "px";
		}
	}

	private void showImage(final TreeItem treeItem, final AbstractImagePrototype proto) {
		final Element holder = treeItem.getImageHolderElement();
		final Element child = DOM.getFirstChild(holder);
		if (child == null) {
	...
	private void updateAriaAttributes() {
		...
		final Element curSelectionContentElem = curSelection.getContentElem();
		...
		final int curSelectionIndex = curSelectionParent.getChildIndex(curSelection);
