...
public class TreeItem extends UIObject implements IsTreeItem, HasTreeItems, HasHTML, HasSafeHtml {
	...
	public static class TreeItemImpl {
		...
		void convertToFullNode(final TreeItem item) {
			if (item.imageHolder == null) {
				// Extract the Elements from the object
				final Element itemTable = DOM.clone(BASE_INTERNAL_ELEM, true);
				DOM.appendChild(item.getElement(), itemTable);
				final Element tr = DOM.getFirstChild(DOM.getFirstChild(itemTable));
				final Element tdImg = DOM.getFirstChild(tr);
				final Element tdContent = DOM.getNextSibling(tdImg);

				DOM.setStyleAttribute(tdImg, "width", "16px");
		...
		void initializeClonableElements() {
			if (GWT.isClient()) {
				...
				final Element contentElem = DOM.createDiv();
				final Element tbody = DOM.createTBody(), tr = DOM.createTR();
				final Element tdImg = DOM.createTD(), tdContent = DOM.createTD();
	...
	private static class TreeItemAnimation extends Animation {
		...
		protected void onUpdate(final double progress) {
			...
			final int scrollWidth = DOM.getElementPropertyInt(curItem.childSpanElem, "scrollWidth");
	...
	private Element childSpanElem, imageHolder;
	...
	TreeItem(final boolean isRoot) {
		this.isRoot = isRoot;
		final Element elem = DOM.clone(BASE_BARE_ELEM, true);
		setElement(elem);
	...
	public TreeItem addTextItem(final String itemText) {
		final TreeItem ret = new TreeItem();
	...
	void addTreeItems(final List<TreeItem> accum) {
		final int size = getChildCount();
		for (int i = 0; i < size; i++) {
			final TreeItem item = children.get(i);
			accum.add(item);
	...
	void setParentItem(final TreeItem parent) {
		this.parent = parent;
	}
