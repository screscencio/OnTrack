 ...
 * <p>
 * MenuBar elements in UiBinder template files can have a <code>vertical</code> boolean attribute (which defaults to false), and may have only MenuItem elements
 * as children. MenuItems may contain HTML and MenuBars.
 * <p>
 * For example:
 * 
 * <pre>
...
public class MenuBar extends Widget implements PopupListener, HasAnimation,
	 ...
	 */
	private final ArrayList<UIObject> allItems = new ArrayList<UIObject>();
	 ...
	 */
	public MenuBar(final boolean vertical) {
	 ...
	 */
	@Deprecated
	public MenuBar(final boolean vertical, final MenuBarImages images) {
	 ...
	 */
	public MenuBar(final boolean vertical, final Resources resources) {
		init(vertical,
	 ...
	 */
	@Deprecated
	public MenuBar(final MenuBarImages images) {
		this(false, images);
	}
	 ...
	 * @param resources a bundle that provides images for this menu
	 */
	public MenuBar(final Resources resources) {
	 ...
	 */
	public MenuItem addItem(final SafeHtml html, final Command cmd) {
	 ...
	 * @return the {@link MenuItem} object created
	 */
	public MenuItem addItem(final SafeHtml html, final MenuBar popup) {
	...
	public void clearItems() {
		...
		for (final UIObject item : allItems) {
	 ...
	 */
	public void closeAllChildren(final boolean focus) {
	 ...
	 */
	public int getItemIndex(final MenuItem item) {
	 ...
	 *         MenuBar
	 */
	public int getSeparatorIndex(final MenuItemSeparator item) {
		return allItems.indexOf(item);
	}
	 ...
	 *             range
	 */
	public MenuItem insertItem(final MenuItem item, final int beforeIndex)
	 ...
	 *             range
	 */
	public MenuItemSeparator insertSeparator(final int beforeIndex) {
	 ...
	 *             range
	 */
	public MenuItemSeparator insertSeparator(final MenuItemSeparator separator,
			final int beforeIndex) throws IndexOutOfBoundsException {
		// Check the bounds
		if (beforeIndex < 0 || beforeIndex > allItems.size()) { throw new IndexOutOfBoundsException(); }
	...
	public void selectItem(final MenuItem item) {
		...
		if (selectedItem != null) {
			...
			if (vertical) {
				final Element tr = DOM.getParent(selectedItem.getElement());
				if (DOM.getChildCount(tr) == 2) {
					final Element td = DOM.getChild(tr, 1);
		...
		if (item != null) {
			...
			// Set the style of the submenu indicator
			if (vertical) {
				final Element tr = DOM.getParent(item.getElement());
				if (DOM.getChildCount(tr) == 2) {
					final Element td = DOM.getChild(tr, 1);
	 ...
	 */
	public void setFocusOnHoverEnabled(final boolean enabled) {
		focusOnHover = enabled;
	}

	/**
	 * Returns a list containing the <code>MenuItem</code> objects in the menu
	 * bar. If there are no items in the menu bar, then an empty <code>List</code> object will be returned.
	 * 
	 ...
	 * by the user. If none of the items in the menu are currently selected, then <code>null</code> will be returned.
	 * 
	 ...
	 * otherwise.
	 */
	void doItemAction(final MenuItem item, final boolean fireCommand, final boolean focus) {
	...
	void itemOver(final MenuItem item, final boolean focus) {
		if (item == null) {
			// Don't clear selection if the currently selected item's menu is showing.
			if ((selectedItem != null)
					&& (shownChildMenu == selectedItem.getSubMenu())) { return; }
		}
	 ...
	 */
	void setMenuItemDebugIds(final String baseID) {
		int itemCount = 0;
	 ...
	 */
	void updateSubmenuIcon(final MenuItem item) {
		// The submenu icon only applies to vertical menus
		if (!vertical) { return; }

		// Get the index of the MenuItem
		final int idx = allItems.indexOf(item);
		if (idx == -1) { return; }

		final Element container = getItemContainerElement();
		final Element tr = DOM.getChild(container, idx);
		final int tdCount = DOM.getChildCount(tr);
	...
	/**
	 * Physically add the td element of a {@link MenuItem} or {@link MenuItemSeparator} to this {@link MenuBar}.
	 * 
	 * @param beforeIndex the index where the separator should be inserted
	 * @param tdElem the td element to be added
	 */
	private void addItemElement(final int beforeIndex, final Element tdElem) {
		if (vertical) {
			...
			final Element tr = DOM.getChild(body, 0);
	 ...
	 * @param focus true to move focus to the parent
	 */
	private void close(final boolean focus) {
		if (parentMenu != null) {
	...
	private void eatEvent(final Event event) {
		DOM.eventCancelBubble(event, true);
		DOM.eventPreventDefault(event);
	}

	private MenuItem findItem(final Element hItem) {
		for (final MenuItem item : items) {
	...
	private void init(final boolean vertical, final AbstractImagePrototype subMenuIcon) {
		...
		final Element outer = FocusPanel.impl.createFocusable();
		...
		sinkEvents(Event.ONCLICK | Event.ONMOUSEOVER | Event.ONMOUSEOUT
				| Event.ONFOCUS | Event.ONKEYDOWN | Event.ONKEYUP); // Changed because of the need of capturing the enter on the keyup event.
	 ...
	 */
	private void onHide(final boolean focus) {
		if (shownChildMenu != null) {
			shownChildMenu.onHide(focus);
			popup.hide();
			if (focus) {
	...
	super.onPreviewNativeEvent(event);
		...
		final String primaryStyleName = getStylePrimaryName();
	 ...
	 */
	private boolean removeItemElement(final UIObject item) {
		final int idx = allItems.indexOf(item);
