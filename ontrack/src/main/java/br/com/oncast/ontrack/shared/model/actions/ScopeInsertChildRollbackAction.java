...
public class ScopeInsertChildRollbackAction implements ScopeAction {
	...
	public ModelAction execute(final ProjectContext context) throws UnableToCompleteActionException {
		...
		final String description = new ScopeRepresentationBuilder(selectedScope).includeEverything().toString();

		executeSubActions(context, selectedScope);
		parent.remove(selectedScope);

		return new ScopeInsertChildAction(parentScopeId, referenceId, description);
	}

	private void executeSubActions(final ProjectContext context, final Scope selectedScope) throws UnableToCompleteActionException {
