...
public class AuthorizationManagerImpl implements AuthorizationManager {
	...
	public void authorize(final long projectId, final String userEmail, final boolean shouldSendMailNotification)
		...
		try {
			...
			throw new UnableToAuthorizeUserException("It was not possible to authorize the user '" + userEmail + "' for the project.");
		}
		catch (final NoResultFoundException e) {
			throw new UnableToAuthorizeUserException("It was not possible to authorize the user '" + userEmail
					+ "' for the project: The project wasn't avaiable.");
		}
	}

	private boolean validateUserAndItsProjectAccessAuthorization(final long projectId, final String userEmail) throws PersistenceException,
	...
	void validateAndUpdateUserUserInvitaionQuota(final String userToBeAuthorizedEmail, final User requestingUser)
		...
		if (!requestingUser.getEmail().equals(userToBeAuthorizedEmail)) {
			...
			if (invitationQuota <= 0) throw new UnableToAuthorizeUserException("The current user's invitation quota has exceeded.");
	...
	private void sendMailNotification(final long projectId, final String userEmail, final boolean isNewUser, final User authenticatedUser)
