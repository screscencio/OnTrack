...
public class ScopeParser {
	...
	public ScopeParser(final String description) throws MalformedScopeException {
		if (description.trim().equals("")) {
			scopeDescription = "";
			return;
		}

		final MatchResult singleReleaseMatchResult = ONLY_ONE_RELEASE_AT_A_TIME_REGEX.exec(description.trim());
		if (singleReleaseMatchResult != null) throw new MalformedScopeException("You cannot set more than one release at a time.");

		final MatchResult matchResult = SCOPE_REG_EX.exec(description);
