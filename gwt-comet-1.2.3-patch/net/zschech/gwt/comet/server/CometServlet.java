...
/**
 * This is the base class for application's Comet servlets. To process a Comet
 * request override {@link #doComet(CometServletResponse)} and send messages by
 * calling {@link CometServletResponse#write(Serializable)} or enqueue messages
 * using {@link CometServletResponse#getSession()} and
...
public class CometServlet extends HttpServlet {
	...
	private CometServletResponseImpl createCometServletResponse(HttpServletRequest request, HttpServletResponse response, SerializationPolicy serializationPolicy,
	...
	private void doCometImpl(CometServletResponseImpl response) throws IOException {
		...
		// at this point the application may have spawned threads to process
		// this response
	...
	/**
	 * Override this method to process a new comet request. All required
	 * information from the {@link HttpServletRequest} must be retrieved
	 * {@link CometServletResponse#getRequest()} in this method as it will not
	 * be available after this method returns and the request is suspended. This
	 * method may write data to the Comet response but should not block. Writing
	 * data from this method before the request is suspended can improve the
	 * efficiency because padding data may not be needed to cause the browser to
	...
	/**
	 * Override this method to be notified of the Comet connection being
	 * terminated.
	 * 
	 * @param cometResponse
	...
	/**
	 * Override this method to override the requested heartbeat. By default only
	 * requested heartbeats > this.heartbeat are allowed.
	 * 
	...
	public static CometSession getCometSession(HttpSession httpSession, Queue<Serializable> queue) {
		return getCometSession(httpSession, true, CometSession.HTTP_SESSION_KEY, queue);
	}

	public static CometSession getCometSession(HttpSession httpSession, boolean create) {
		return getCometSession(httpSession, create, CometSession.HTTP_SESSION_KEY, create ? new ConcurrentLinkedQueue<Serializable>() : null);
	}

	public static CometSession getCometSession(HttpSession httpSession, boolean create,String sessionID) {
		return getCometSession(httpSession, create,sessionID, create ? new ConcurrentLinkedQueue<Serializable>() : null);
	}

	public static CometSession getCometSession(HttpSession httpSession, boolean create,  String sessionID, Queue<Serializable> queue) {
		synchronized (httpSession) {
			CometSession session = (CometSession) httpSession.getAttribute(sessionID);
			if (session == null) {
				if (create) {
					session = new CometSessionImpl(httpSession, queue, AsyncServlet.initialize(httpSession.getServletContext()), sessionID);
