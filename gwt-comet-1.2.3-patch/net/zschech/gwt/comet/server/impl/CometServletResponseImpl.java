...
public abstract class CometServletResponseImpl implements CometServletResponse {
	...
	public synchronized CometSession getSession(boolean create) {
		return getSession(create, getRequest().getParameter(COMET_SESSION_ID_PARAM));
	}
	
	@Override
	public CometSession getSession(boolean create, String sessionID) {
		if (suspended) throw new IllegalStateException("CometSession can not be accessed after the CometServletResponse has been suspended.");
		if (session != null) return session;
		
		HttpSession httpSession = getRequest().getSession(create);
		if (httpSession == null) return null;
		
		session = (CometSessionImpl) CometServlet.getCometSession(httpSession, create, sessionID);
		if (create) {
